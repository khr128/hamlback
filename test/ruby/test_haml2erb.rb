...
#    File.delete(@output_file_name) if File.exists?(@output_file_name)
  ...
  def test_simplest_div_with_html_comment
   `#{@ha2er_exe} < #{File.join(@cur_dir, "haml/simplest_div_with_html_comment.html.haml")} > #{@output_file_name}`

    expected_file_name = File.join(@cur_dir, "expected/simplest_div_with_html_comment.html.erb")
    diff = `diff #{@output_file_name} #{expected_file_name}`
    assert_equal 0, $?.exitstatus, diff
    assert diff.empty?, diff
  end
  ...
  def test_simplest_div_with_plain_text
   `#{@ha2er_exe} < #{File.join(@cur_dir, "haml/simplest_div_with_plain_text.html.haml")} > #{@output_file_name}`

    expected_file_name = File.join(@cur_dir, "expected/simplest_div_with_plain_text.html.erb")
    diff = `diff #{@output_file_name} #{expected_file_name}`
    assert_equal 0, $?.exitstatus, diff
    assert diff.empty?, diff
  end
  ...
  def test_simplest_div_with_escaped_plain_text
   `#{@ha2er_exe} < #{File.join(@cur_dir, "haml/simplest_div_with_escaped_plain_text.html.haml")} > #{@output_file_name}`

    expected_file_name = File.join(@cur_dir, "expected/simplest_div_with_escaped_plain_text.html.erb")
    diff = `diff #{@output_file_name} #{expected_file_name}`
    assert_equal 0, $?.exitstatus, diff
    assert diff.empty?, diff
  end
  ...
  def test_ps_users_index
   `#{@ha2er_exe} < #{File.join(@cur_dir, "haml/ps_users/index.html.haml")} > #{@output_file_name}`

    expected_file_name = File.join(@cur_dir, "expected/ps_users/index.html.erb")
    diff = `diff #{@output_file_name} #{expected_file_name}`
    assert_equal 0, $?.exitstatus, diff
    assert diff.empty?, diff
  end
